...
# Wichtig: Finder/LaunchServices startet nicht aus dem Projektordner -> relative assets müssen trotzdem passen
rm -f "$APP_BUNDLE/Contents/MacOS/assets" 2>/dev/null || true
...
SFML_PREFIX="$BREW_PREFIX/opt/sfml/lib"
SFML_ABS=("${(@f)$(otool -L "$EXEC_PATH" | awk '{print $1}' | grep "^${SFML_PREFIX}/libsfml-.*\\.dylib$" || true)}")
...
echo "== Final pass: ensure no Homebrew absolute paths remain in main executable =="
deps=$(otool -L "$EXEC_PATH" | awk '{print $1}' | tail -n +2 | grep "^$BREW_PREFIX" || true)
for dep in ${(f)deps}; do
  base="${dep:t}"
  if [[ -f "$FW_DIR/$base" ]]; then
    install_name_tool -change "$dep" "@rpath/$base" "$EXEC_PATH" 2>/dev/null || true
  fi
done
